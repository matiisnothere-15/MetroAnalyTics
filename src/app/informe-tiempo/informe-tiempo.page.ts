import { Component, OnInit, ViewChild, ElementRef } from '@angular/core';
import { CommonModule } from '@angular/common';
import { FormsModule } from '@angular/forms';
import { IonicModule, NavController } from '@ionic/angular';

import * as pdfMake from "pdfmake/build/pdfmake";
import * as pdfFonts from 'pdfmake/build/vfs_fonts';
(<any>pdfMake).vfs = pdfFonts.pdfMake.vfs;
import Chart from 'chart.js/auto';

//Dependencias para descargar informe en Android y IOS
import { Filesystem, Directory, Encoding } from '@capacitor/filesystem';
import { Firebase } from 'src/app.firebase';
import { DocumentData } from 'firebase/firestore';

@Component({
  selector: 'app-informe-tiempo',
  templateUrl: './informe-tiempo.page.html',
  styleUrls: ['./informe-tiempo.page.scss'],
  standalone: true,
  imports: [IonicModule, CommonModule, FormsModule]
})
export class InformeTiempoPage implements OnInit {

  @ViewChild('chartL1', { static: true }) chartL1!: ElementRef<HTMLCanvasElement>;
  @ViewChild('chartL2', { static: true }) chartL2!: ElementRef<HTMLCanvasElement>;
  @ViewChild('chartL3', { static: true }) chartL3!: ElementRef<HTMLCanvasElement>;
  @ViewChild('chartL4', { static: true }) chartL4!: ElementRef<HTMLCanvasElement>;
  @ViewChild('chartL4A', { static: true }) chartL4A!: ElementRef<HTMLCanvasElement>;
  @ViewChild('chartL5', { static: true }) chartL5!: ElementRef<HTMLCanvasElement>;
  @ViewChild('chartL6', { static: true }) chartL6!: ElementRef<HTMLCanvasElement>;
  
  public chartL1grafico: any;
  public chartL2grafico: any;
  public chartL3grafico: any;
  public chartL4grafico: any;
  public chartL4Agrafico: any;
  public chartL5grafico: any;
  public chartL6grafico: any;

  constructor(private navCtrl: NavController) { }

  app = new Firebase()

  ngOnInit() {
    this.app.inicializarFirebase();
    this.generarGraficoTiempo();
  }

  //====================================Funciones para generar informe por cada línea==========================================
  generarInformeTiempoL1() {
    const tiempoTranscurrido = Date.now();
    const hoy = new Date(tiempoTranscurrido);

    var dd = {
      content: [
        {
          image: 'data:image/png;base64,',
        },
        {
          text: `\n\nFecha de generación: ${hoy.toLocaleDateString()}`,
          fontSize: 10
        },
        {
          text: `\n\n\nTiempo de traslado entre estaciones de la linea 1\n\n\n`,
          fontSize: 15,
          bold: true,
        },
        {
          image: this.chartL1.nativeElement.toDataURL(),
          width: 500
        },
      ],

      
    }

    const documentDefinition = dd;
    pdfMake.createPdf(documentDefinition).download('(L1) Tiempo de traslado ente estaciones.pdf');
  }

  generarInformeTiempoL2() {
    const tiempoTranscurrido = Date.now();
    const hoy = new Date(tiempoTranscurrido);

    var dd = {
      content: [
        {
          image: 'data:image/png;base64,',
        },
        {
          text: `\n\nFecha de generación: ${hoy.toLocaleDateString()}`,
          fontSize: 10
        },
        {
          text: `\n\n\nTiempo de traslado entre estaciones de la linea 2\n\n\n`,
          fontSize: 15,
          bold: true,
        },
        {
          image: this.chartL2.nativeElement.toDataURL(),
          width: 500
        },
      ],

      
    }

    const documentDefinition = dd;
    pdfMake.createPdf(documentDefinition).download('(L2) Tiempo de traslado ente estaciones.pdf');
  }

  generarInformeTiempoL3() {
    const tiempoTranscurrido = Date.now();
    const hoy = new Date(tiempoTranscurrido);

    var dd = {
      content: [
        {
          image: 'data:image/png;base64,',
        },
        {
          text: `\n\nFecha de generación: ${hoy.toLocaleDateString()}`,
          fontSize: 10
        },
        {
          text: `\n\n\nTiempo de traslado entre estaciones de la linea 3\n\n\n`,
          fontSize: 15,
          bold: true,
        },
        {
          image: this.chartL3.nativeElement.toDataURL(),
          width: 500
        },
      ],

      
    }

    const documentDefinition = dd;
    pdfMake.createPdf(documentDefinition).download('(L3) Tiempo de traslado ente estaciones.pdf');
  }

  generarInformeTiempoL4() {
    const tiempoTranscurrido = Date.now();
    const hoy = new Date(tiempoTranscurrido);

    var dd = {
      content: [
        {
          image: 'data:image/png;base64,',
        },
        {
          text: `\n\nFecha de generación: ${hoy.toLocaleDateString()}`,
          fontSize: 10
        },
        {
          text: `\n\n\nTiempo de traslado entre estaciones de la linea 4\n\n\n`,
          fontSize: 15,
          bold: true,
        },
        {
          image: this.chartL4.nativeElement.toDataURL(),
          width: 500
        },
      ],

      
    }

    const documentDefinition = dd;
    pdfMake.createPdf(documentDefinition).download('(L4) Tiempo de traslado ente estaciones.pdf');
  }

  generarInformeTiempoL4A() {
    const tiempoTranscurrido = Date.now();
    const hoy = new Date(tiempoTranscurrido);

    var dd = {
      content: [
        {
          image: 'data:image/png;base64,',
        },
        {
          text: `\n\nFecha de generación: ${hoy.toLocaleDateString()}`,
          fontSize: 10
        },
        {
          text: `\n\n\nTiempo de traslado entre estaciones de la linea 4A\n\n\n`,
          fontSize: 15,
          bold: true,
        },
        {
          image: this.chartL4A.nativeElement.toDataURL(),
          width: 500
        },
      ],

      
    }

    const documentDefinition = dd;
    pdfMake.createPdf(documentDefinition).download('(L4A) Tiempo de traslado ente estaciones.pdf');
  }

  generarInformeTiempoL5() {
    const tiempoTranscurrido = Date.now();
    const hoy = new Date(tiempoTranscurrido);

    var dd = {
      content: [
        {
          image: 'data:image/png;base64,',
        },
        {
          text: `\n\nFecha de generación: ${hoy.toLocaleDateString()}`,
          fontSize: 10
        },
        {
          text: `\n\n\nTiempo de traslado entre estaciones de la linea 5\n\n\n`,
          fontSize: 15,
          bold: true,
        },
        {
          image: this.chartL5.nativeElement.toDataURL(),
          width: 500
        },
      ],

      
    }

    const documentDefinition = dd;
    pdfMake.createPdf(documentDefinition).download('(L5) Tiempo de traslado ente estaciones.pdf');
  }

  generarInformeTiempoL6() {
    const tiempoTranscurrido = Date.now();
    const hoy = new Date(tiempoTranscurrido);

    var dd = {
      content: [
        {
          image: 'data:image/png;base64,',
        },
        {
          text: `\n\nFecha de generación: ${hoy.toLocaleDateString()}`,
          fontSize: 10
        },
        {
          text: `\n\n\nTiempo de traslado entre estaciones de la linea 6\n\n\n`,
          fontSize: 15,
          bold: true,
        },
        {
          image: this.chartL6.nativeElement.toDataURL(),
          width: 500
        },
      ],

      
    }

    const documentDefinition = dd;
    pdfMake.createPdf(documentDefinition).download('(L6) Tiempo de traslado ente estaciones.pdf');
  }

  irInsertarTiempotras() {
    this.navCtrl.navigateForward('/insertar-tiempo-tras');
  }
  //========================Generar gráficos de tiempo de traslado entre estaciones por cada línea=============================
  async generarGraficoTiempo() {

    let datosL1: DocumentData = await this.app.obtenerDatosTiempo('1');

    this.chartL1grafico = new Chart(this.chartL1.nativeElement, {
      type: 'bar',
      data: {
        labels: [`${datosL1[0].estacion_origen} y ${datosL1[0].estacion_destino}`, `${datosL1[1].estacion_origen} y ${datosL1[1].estacion_destino}`],
        datasets: [{
          label: 'Tiempo en minutos',
          data: [datosL1[0].tiempo, datosL1[1].tiempo],
          borderWidth: 1,
          backgroundColor: '#ec1d25'
        }]
      },
      options: {
        scales: {
          y: {
            beginAtZero: true
          }
        }
      }
    });

    let datosL2: DocumentData = await this.app.obtenerDatosTiempo('2');

    this.chartL2grafico = new Chart(this.chartL2.nativeElement, {
      type: 'bar',
      data: {
        labels: [`${datosL2[0].estacion_origen} y ${datosL2[0].estacion_destino}`, `${datosL2[1].estacion_origen} y ${datosL2[1].estacion_destino}`],
        datasets: [{
          label: 'Tiempo en minutos',
          data: [datosL2[0].tiempo, datosL2[1].tiempo],
          borderWidth: 1,
          backgroundColor: '#f7941d'
        }]
      },
      options: {
        scales: {
          y: {
            beginAtZero: true
          }
        }
      }
    });

    let datosL3: DocumentData = await this.app.obtenerDatosTiempo('3');

    this.chartL3grafico = new Chart(this.chartL3.nativeElement, {
      type: 'bar',
      data: {
        labels: [`${datosL3[0].estacion_origen} y ${datosL3[0].estacion_destino}`, `${datosL3[1].estacion_origen} y ${datosL3[1].estacion_destino}`],
        datasets: [{
          label: 'Tiempo en minutos',
          data: [datosL3[0].tiempo, datosL3[1].tiempo],
          borderWidth: 1,
          backgroundColor: '#994807'
        }]
      },
      options: {
        scales: {
          y: {
            beginAtZero: true
          }
        }
      }
    });

    let datosL4: DocumentData = await this.app.obtenerDatosTiempo('4');

    this.chartL4grafico = new Chart(this.chartL4.nativeElement, {
      type: 'bar',
      data: {
        labels: [`${datosL4[0].estacion_origen} y ${datosL4[0].estacion_destino}`, `${datosL4[1].estacion_origen} y ${datosL4[1].estacion_destino}`],
        datasets: [{
          label: 'Tiempo en minutos',
          data: [datosL4[0].tiempo, datosL4[1].tiempo],
          borderWidth: 1,
          backgroundColor: '#2e3192'
        }]
      },
      options: {
        scales: {
          y: {
            beginAtZero: true
          }
        }
      }
    });

    let datosL4A: DocumentData = await this.app.obtenerDatosTiempo('4A');

    this.chartL4Agrafico = new Chart(this.chartL4A.nativeElement, {
      type: 'bar',
      data: {
        labels: [`${datosL4A[0].estacion_origen} y ${datosL4A[0].estacion_destino}`, `${datosL4A[1].estacion_origen} y ${datosL4A[1].estacion_destino}`],
        datasets: [{
          label: 'Tiempo en minutos',
          data: [datosL4A[0].tiempo, datosL4A[1].tiempo],
          borderWidth: 1,
          backgroundColor: '#05adf4'
        }]
      },
      options: {
        scales: {
          y: {
            beginAtZero: true
          }
        }
      }
    });

    let datosL5: DocumentData = await this.app.obtenerDatosTiempo('5');

    this.chartL5grafico = new Chart(this.chartL5.nativeElement, {
      type: 'bar',
      data: {
        labels: [`${datosL5[0].estacion_origen} y ${datosL5[0].estacion_destino}`, `${datosL5[1].estacion_origen} y ${datosL5[1].estacion_destino}`],
        datasets: [{
          label: 'Tiempo en minutos',
          data: [datosL5[0].tiempo, datosL5[1].tiempo],
          borderWidth: 1,
          backgroundColor: '#0b9444'
        }]
      },
      options: {
        scales: {
          y: {
            beginAtZero: true
          }
        }
      }
    });

    let datosL6: DocumentData = await this.app.obtenerDatosTiempo('6');

    this.chartL6grafico = new Chart(this.chartL6.nativeElement, {
      type: 'bar',
      data: {
        labels: [`${datosL6[0].estacion_origen} y ${datosL6[0].estacion_destino}`, `${datosL6[1].estacion_origen} y ${datosL6[1].estacion_destino}`],
        datasets: [{
          label: 'Tiempo en minutos',
          data: [datosL6[0].tiempo, datosL6[1].tiempo],
          borderWidth: 1,
          backgroundColor: '#a35db5'
        }]
      },
      options: {
        scales: {
          y: {
            beginAtZero: true
          }
        }
      }
    });

  }

  volverAtras() {
    this.navCtrl.navigateForward('/home');
  }

}
